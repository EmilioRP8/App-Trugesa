...
porcentajeoperador = 0.10
cotizaciones = [0.10, 0.20, 0.30, 0.40]

st.title("Cotizador de Fletes - Turismo")

# Establecer el tipo de servicio fijo por página
st.session_state['modo_servicio'] = "Turismo"

# Construir ruta segura a la imagen
...
with st.sidebar:
    if os.path.exists(img_path):
        ...
        st.warning("⚠️ No se encontró el logo 'tr.png'")
        # Opcional: Imagen de respaldo en línea
        # st.image("https://via.placeholder.com/300x100.png?text=Logo+Trugesa", use_container_width=True)
...
for i in range(st.session_state['contador_formularios']):
    ...
    if submitted:
        ...
        valores = [cotizacion_id, lugar_partida, destino, "Autobus", km_ida_vuelta,
        ...
        st.session_state['finalizado_guardado'] = False
        st.success(f"✅ Cotización agregada correctamente para el servicio '{st.session_state['modo_servicio']}'.")
